...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val predictionResultTopic = "flight_delay_classification_result"

    // Publicar los resultados en Kafka
    val kafkaOutput = finalPredictions
      .selectExpr("UUID", "to_json(struct(*)) AS value") // Convertir DataFrame a JSON para Kafka
      .select($"value")
    ...
    val kafkaStreamWriter = kafkaOutput
      ...
      .option("topic", predictionResultTopic)
