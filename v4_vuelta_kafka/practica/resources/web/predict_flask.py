...
db = client['agile_data_science']  # Nombre de la base de datos
coleccion = db['origin_dest_distances']  # Nombre de la colección
...
PREDICTION_RESULT_TOPIC = 'flight_delay_classification_result'

consumer = KafkaConsumer(
    'flight_delay_classification_result',  # Topic
    bootstrap_servers=['kafka:9092'],api_version=(0,10),  # Dirección de Kafka
    group_id='flight-delay-group',  # Grupo de consumidores 
...
def classify_flight_delays_realtime():
  ...
  producer.send(PREDICTION_RESULT_TOPIC, message_bytes)
...
# Diccionarios para almacenar las solicitudes y predicciones
flight_requests = {}  # Almacena las solicitudes sin la predicción
predictions = {}  # Almacena las predicciones completas
...
def classify_flight_delays_realtime_response(unique_id):
    ...
    # Buscar predicción asociada al UUID
    prediction = predictions.get(unique_id, None)

    # Preparar respuesta inicial
    response = {"status": "WAIT", "id": unique_id}

    if prediction:
        # Agregar detalles completos de la predicción
        response["status"] = "OK"
        response["prediction"] = prediction

        # Agregar delay_status categorizado
        delay_status = "Unknown"
        if prediction["Prediction"] == 0.0:
            delay_status = "Early (15+ Minutes Early)"
        elif prediction["Prediction"] == 1.0:
            delay_status = "Slightly Early (0-15 Minute Early)"
        elif prediction["Prediction"] == 2.0:
            delay_status = "Slightly Late (0-30 Minute Delay)"
        elif prediction["Prediction"] == 3.0:
            delay_status = "Very Late (30+ Minutes Late)"    

        response["delay_status"] = delay_status

    print(f"Response sent: {response}")
    return jsonify(response)

# Función para consumir predicciones de Kafka en un hilo separado
# Función para consumir predicciones de Kafka en un hilo separado
def consume_predictions():
    """Consumes messages from Kafka and processes them."""
    for message in consumer:
        try:
            # Decodifica el mensaje de Kafka
            prediction = json.loads(message.value.decode('utf-8'))
            print(f"Received message: {prediction}")
            logging.debug(f"Received message: {prediction}")

            # Verifica si el mensaje tiene un UUID
            uuid = prediction.get("UUID")
            if not uuid:
                print("Error: Message without UUID. Ignored.")
                continue

            # Verifica si es una predicción
            if "Prediction" in prediction:
                # Almacena la predicción
                predictions[uuid] = prediction
                print(f"Stored prediction for UUID: {uuid}")
                logging.info(f"Stored prediction for UUID: {uuid}")
            else:
                # Es una solicitud, almacénala
                flight_requests[uuid] = prediction
                print(f"Stored flight request for UUID: {uuid}")

        except Exception as e:
            print(f"Error processing message: {e}")
            logging.error(f"Error processing message: {e}")

# Crear un hilo para ejecutar el consumidor de Kafka
