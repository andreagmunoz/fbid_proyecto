...
def on_time_performance():
  ...
  flight_num = request.args.get('FlightNum')
   ...
   ## Cassandra ##
  query = """
    SELECT * FROM on_time_performance WHERE Carrier = %s AND FlightDate = %s AND FlightNum = %s;
    """
  result = session.execute(query, (carrier, flight_date, flight_num))
    
    # Extrae el primer resultado si existe
  flight = result.one()

  if flight:
        # Mapea el resultado a un diccionario, si es necesario
        flight = dict(flight._asdict())
  else:
        flight = {}
...
def list_flights(origin, dest, flight_date):
  ## Cassandra ##
  page_size = 100  # Cambia esto por el tamaño de página que desees
  query = """
    SELECT * FROM on_time_performance WHERE Origin = %s AND Dest = %s AND FlightDate = %s;
    """
  statement = SimpleStatement(query, fetch_size=page_size)
  result = session.execute(statement, (origin, dest, flight_date))
    
    # Resultados
  flights = [row._asdict() for row in result]
...
def classify_flight_delays():
  ...
  ##Cassandra##
  # Aquí insertamos los datos en Cassandra
    query = """
    INSERT INTO prediction_tasks (DepDelay, Carrier, FlightDate, Dest, FlightNum, Origin, Timestamp)
    VALUES (%s, %s, %s, %s, %s, %s, %s);
    """
    session.execute(query, (prediction_features['DepDelay'], prediction_features['Carrier'], 
                            prediction_features['FlightDate'], prediction_features['Dest'], 
                            prediction_features['FlightNum'], prediction_features['Origin'], 
                            prediction_features['Timestamp']))
