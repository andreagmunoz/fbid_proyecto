...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // spark.sparkContext.setLogLevel("WARN")
    ...
    val base_path= "/spark/work-dir/practica"
    ...
    //   .option("table", "distances")
    //   .outputMode("append")complete


    // val dfprev = finalPredictions
    ...
    //    Mongo.save(batchDF,writeConfig)
    // }.start()

    // For writing to Cassandra
    // val dataStreamWriter = finalPredictions
    //   .saveToCassandra()
    //   .option("spark.cassandra.connection.uri", "cassandra:9042")
    //   .option("checkpointLocation", "/tmp")
    //   .option("spark.cassandra.collection", "flight_delay_classification_response")
    //   .outputMode("append")
