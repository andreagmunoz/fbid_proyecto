...
def connect_to_cassandra():
    cluster_address = "cassandra"
    port = 9042
    cluster = Cluster([cluster_address], port=port)
    

    # Intentar conectarse hasta que Cassandra esté disponible
    while True:
        try:
            ...
            print("Esperando a que Cassandra esté disponible...")
            time.sleep(5)  # Espera 5 segundos antes de intentar nuevamente

session= connect_to_cassandra()
cassandra = connect_to_cassandra()
keyspace= "agile_data_science"

# # Configuración de conexión
# cluster_address = "cassandra"  # Nombre del contenedor o dirección IP de Cassandra
# port = 9042  # Puerto predeterminado de Cassandra
# keyspace = "agile_data_science"
...
def on_time_performance():
  ...
  session = cassandra.connect(keyspace=keyspace)

  statement = session.prepare(f"SELECT * FROM ${keyspace}.on_time_performance WHERE carrier=? AND flightdate=? AND flightnum=? LIMIT 1")

  flight = session.execute(statement, (carrier, flight_date, flight_num))
  
  # flight = cassandra.agile_data_science.on_time_performance.find_one({
  #   'Carrier': carrier,
...
def list_flights(origin, dest, flight_date):
  ...
  flights = cassandra.agile_data_science.on_time_performance.find(
    {
      'Origin': origin,
      'Dest': dest,
      'FlightDate': flight_date
...
def total_flights():
  total_flights = cassandra.agile_data_science.flights_by_month.find({}, 
    sort = [
      ('Year', 1),
      ('Month', 1)
    ])
...
def airplane_manufacturers_chart():
  mfr_chart = cassandra.agile_data_science.airplane_manufacturer_totals.find_one()
...
#   flights = cassandra.agile_data_science.flights_per_airplane.find_one(
#     {'TailNum': tail_number}
#   )
#   return render_template(
...
#   airline_summary = cassandra.agile_data_science.airlines.find_one(
#     {'CarrierCode': carrier_code}
#   )
#   airline_airplanes = cassandra.agile_data_science.airplanes_per_carrier.find_one(
...
@app.route("/airlines/")
def airlines():
 # airlines = cassandra.agile_data_science.airplanes_per_carrier.find()
  
  session = cassandra.connect(keyspace=keyspace)
  statement = session.prepare("SELECT carrier from agile_data_science.airplanes")
  
  airlines = session.execute(statement)
...
def regress_flight_delays():
  ...
  prediction_features['Distance'] = predict_utils.get_flight_distance(cassandra, keyspace, str(api_form_values['Origin']), str(api_form_values['Dest']))
...
def classify_flight_delays():
  ...
  prediction_features['Distance'] = predict_utils.get_flight_distance(
    cassandra, keyspace, api_form_values['Origin'],
  ...
  cassandra.agile_data_science.prediction_tasks.insert_one(
    prediction_features
  )
...
def flight_delays_batch_results_page(iso_date):
  ...
  # predictions = cassandra.agile_data_science.prediction_results.find(
  #   {
  #     'Timestamp': {
  #       "$gte": iso_today,
  #       "$lte": iso_tomorrow,
  ...
  session = cassandra.connect(keyspace=keyspace)
  statement = session.prepare("SELECT * from agile_data_science.predictions WHERE timestamp > ? AND timestamp < ?")
  
  predictions = session.execute(statement, (str(iso_today),str(iso_tomorrow)))
...
def classify_flight_delays_realtime():
    """POST API for classifying flight delays"""
    # Utiliza la función para conectarte a Cassandra correctamente
    cassandra_session = connect_to_cassandra()  # Esto debe ser el método correcto de conexión a Cassandra

    # Define los campos del formulario que se van a procesar
    api_field_type_map = {
        "DepDelay": float,
        "Carrier": str,
        "FlightDate": str,
    ...
    # Configura los valores directos (que excluyen la fecha)
    prediction_features = {}
    for key, value in api_form_values.items():
        prediction_features[key] = value

    # Configura los valores derivados
    prediction_features['Distance'] = predict_utils.get_flight_distance(
        cassandra_session, keyspace, api_form_values['Origin'], api_form_values['Dest']
    )

    # Convierte la fecha en DayOfYear, DayOfMonth, DayOfWeek
    date_features_dict = predict_utils.get_regression_date_args(api_form_values['FlightDate'])
    for api_field_name, api_field_value in date_features_dict.items():
        prediction_features[api_field_name] = api_field_value
    ...
    # Envía los datos al productor Kafka (suponiendo que `producer` y `PREDICTION_TOPIC` están definidos)
    message_bytes = json.dumps(prediction_features, cls=predict_utils.UUIDEncoder).encode()
    producer.send(PREDICTION_TOPIC, message_bytes)

    # Responde con el estado OK y el ID único
    response = {"status": "OK", "id": unique_id}
    return json.dumps(response, cls=predict_utils.UUIDEncoder)
...
def classify_flight_delays_realtime_response(unique_id):
    # Consulta a la tabla predictions en Cassandra
    query = SimpleStatement("SELECT * FROM predictions WHERE uuid = %s", fetch_size=1)
    rows = session.execute(query, (uuid.UUID(unique_id),))
    ...
    for row in rows:
        ...
        # Categorización del estado de la predicción
        prediction = row.prediction
        if prediction == 0.0:
            response["delay_status"] = "On Time (0-15 Minute Delay)"
        elif prediction == 1.0:
            response["delay_status"] = "Slightly Late (15-60 Minute Delay)"
        elif prediction == 2.0:
            response["delay_status"] = "Very Late (60+ Minute Delay)"
        else:
            response["delay_status"] = "Unknown"
